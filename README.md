...
###Iteration
C3BT iterates just like a standard BST, except that we have a parent pointer
for each cell, thus backtracking is cell-by-cell and very quick.  After locating
the cell, we follow the usual node-by-node path to locate the predecessor or
successor.
...
of the cell's sub-cells into itself.
...
###Allocation Bitmap
Most processors today have instructions like `clz` and `fls` which can find a
bit 1 very quickly.  If the layout has spare bytes, you may consider using them
as allocation map for the nodes and pointer array.  This will speed up node and
pointer allocation.
